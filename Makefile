...
# NOTE: we cannot build nonroot using custom --network names (only run), only slirp4nets
#       combined with host.containers.internal (to access other containers we need to make it host accessible)
#       if the container wants to access e.g. webproxycache, use host.containers.internal:3128 in proxy settings
#       and make webproxy container host accessible --port 3218:3218
.PHONY: build_arch
build_arch:
	podman build \
		--network=slirp4netns \
