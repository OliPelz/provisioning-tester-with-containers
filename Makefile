...
# ---------------------------------------------------------------------------------------
# Local persistent registry (data stored in ./registry_data)
# ---------------------------------------------------------------------------------------
REGISTRY_NAME=local-registry
REGISTRY_PORT=5000
REGISTRY_DIR=$(PWD)/registry_data
REGISTRY_HOST=localhost:$(REGISTRY_PORT)

# Image names (wired to local registry)
NAME1_IMAGE_NAME=$(REGISTRY_HOST)/ubuntu-image
NAME2_IMAGE_NAME=$(REGISTRY_HOST)/rocky-image
NAME3_IMAGE_NAME=$(REGISTRY_HOST)/arch-image
WEBPROXYCACHE_IMAGE_NAME=$(REGISTRY_HOST)/webproxycache-image
...
# ---------------------------------------------------------------------------------------
.PHONY: registry-up
registry-up:  ### start persistent local registry (data in ./registry_data)
	@mkdir -p $(REGISTRY_DIR)
	@if ! podman container exists $(REGISTRY_NAME); then \
		echo "🚀 Starting local registry $(REGISTRY_NAME) on $(REGISTRY_HOST)"; \
		podman run -d --name $(REGISTRY_NAME) -p 127.0.0.1:$(REGISTRY_PORT):5000 \
		  -v $(REGISTRY_DIR):/var/lib/registry:Z docker.io/library/registry:2 >/dev/null; \
	else \
		if ! podman inspect -f '{{.State.Running}}' $(REGISTRY_NAME) | grep -q true; then \
			echo "▶️  Starting existing registry container $(REGISTRY_NAME)"; \
			podman start $(REGISTRY_NAME) >/dev/null; \
		else \
			echo "ℹ️  Registry $(REGISTRY_NAME) already running"; \
		fi; \
	fi

.PHONY: registry-down
registry-down:  ### stop and remove local registry (keeps data in ./registry_data)
	-@podman rm -f $(REGISTRY_NAME) >/dev/null 2>&1 || true
	@echo "🛑 Registry container removed (data preserved in $(REGISTRY_DIR))"
...
build_arch:
	podman build \
		...
		-t $(NAME3_IMAGE_NAME) . && \
	sudo podman push $(NAME3_IMAGE_NAME)
	
.PHONY: build_webproxycache
build_webproxycache:
	@podman build -f Dockerfile.webproxycache -t $(WEBPROXYCACHE_IMAGE_NAME) . && podman push $(WEBPROXYCACHE_IMAGE_NAME)
...
build_all_images: gen_ssh_container_keys registry-up ### build vm alike container image
...
rsync_copy_cmd_sudo:  ### privileged rsync for MYHOSTNAME= with FROM= and TO=
	@if [ -z "$(MYHOSTNAME)" ]; then \
		echo "$(RED_COLOR)Error: MYHOSTNAME variable is not set. Use make run_cmd MYHOSTNAME=$(NUM) FROM=\"your command\"$(NO_COLOR)" >&2; \
		exit 1; \
	fi
	@if [ -z "$(FROM)" ]; then \
